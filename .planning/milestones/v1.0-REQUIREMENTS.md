# Requirements Archive: v1.0 Parity & Release
...
- [x] **INTRO-01**: User can retrieve a live model's full schema via an `Introspector` that queries `ir.model` / `ir.model.fields`
- [x] **INTRO-02**: Introspection results are served from an `IntrospectionCache` keyed by model name, with a live-bypass option
- [x] **INTRO-03**: User can generate typed Python representations from a live schema via a `CodeGenerator` *(superseded by TYPED-01/02 in v1.1 — TypedDict generator replaced by Pydantic generator, Phase 7)*
- [x] **INTRO-04**: A type mapper translates Odoo field types (`char`, `many2one`, `selection`, …) to Python type hints
...
- [x] **INTRO-06**: Selection fields are emitted as `Literal[...]` type hints, preserving the closed-set constraint
- [x] **INTRO-07**: The `godoo-introspection` package ships a `py.typed` PEP 561 marker
...
*Last updated: 2026-05-22 — all 26 v1 requirements complete; INTRO-* checkboxes/traceability synced at v1.0 milestone close*

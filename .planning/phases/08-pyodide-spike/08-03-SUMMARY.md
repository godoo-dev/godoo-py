...
No new secrets committed. Throwaway Odoo admin credentials were passed as URL query params at run time
only — never committed. The spike FQDN committed to `index.html` defaults is documentation of the
(now-destroyed) spike endpoint; it grants no access (endpoint torn down).
